...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    WNDCLASSEXW wc = { sizeof(wc), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, L"AFKbotLoaderWindow", nullptr };
    ::RegisterClassExW(&wc);
    int screen_width = GetSystemMetrics(SM_CXSCREEN);
    int screen_height = GetSystemMetrics(SM_CYSCREEN);
    var->winapi.hwnd = ::CreateWindowW(wc.lpszClassName, L"AFKbot Loader", WS_POPUP, (GetSystemMetrics(SM_CXSCREEN) / 2) - (SCALE(var->window.size.x) / 2), (GetSystemMetrics(SM_CYSCREEN) / 2) - (SCALE(var->window.size.y) / 2), SCALE(var->window.size.x), SCALE(var->window.size.y), NULL, NULL, wc.hInstance, NULL);
